...
public class StripeService {
  ...
  public StripeSessionId createPayment(User connectedUser,
    ...
    // Get current authenticated user from Spring Security
    Authentication authentication = SecurityContextHolder.getContext().getAuthentication();
    String username = authentication.getName();
    
    // Get the security user to ensure we're using the most up-to-date information
    fr.codecake.ecom.shared.security.User securityUser = securityUserRepository.findByUsername(username)
        .orElseThrow(() -> new RuntimeException("User not found in security repository"));
    
    // Generate a consistent UserPublicId for the security user
    UserPublicId userPublicId = UserAdapter.getUserPublicId(securityUser);
    
    SessionCreateParams.Builder sessionBuilder = SessionCreateParams.builder()
      .setMode(SessionCreateParams.Mode.PAYMENT)
      .putMetadata("user_public_id", userPublicId.value().toString())
      .setCustomerEmail(securityUser.getEmail())
