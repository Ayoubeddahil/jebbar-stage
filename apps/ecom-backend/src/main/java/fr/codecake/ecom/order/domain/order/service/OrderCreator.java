...
public class OrderCreator {
  ...
  public StripeSessionId create(List<Product> productsInformations,
    ...
    if (authentication == null || !authentication.isAuthenticated()) {
      throw new CartPaymentException("User not authenticated");
    }

    // Verify user matches authenticated user
    if (!authentication.getName().equals(connectedUser.getEmail().value())) {
      throw new CartPaymentException("User mismatch");
    }
